    ...
    <div className="min-h-screen pb-10">
      ...
      <nav className="bg-white/10 flex justify-between items-center py-4 px-10">
        ...
        <div className="flex gap-4 items-center">
          <button onClick={handleLogout} className="bg-brand px-4 py-2 rounded-md">
            Logout
          </button>
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-6 w-[90%] mx-auto mt-8">
        {[
          ...
          <div key={i} className="bg-white/10 p-6 text-center rounded-xl" onClick={() => { if (s.title === "Announcements") { setShowAnnouncements(true); fetchAnnouncements(); } }}>
            <h3>{s.title}</h3>
        ...
        {requests.map((r) => (
          <div key={r.id} className="flex justify-between bg-white/10 p-3 mb-2 rounded">
            ...
            <div className="space-x-2">
              <button onClick={() => approveRequest(r.id)} className="bg-green-400 px-3 py-1 rounded">
                Approve
              </button>
              <button onClick={() => rejectRequest(r.id)} className="bg-red-400 px-3 py-1 rounded">
                Reject
              </button>
            </div>
          </div>
        ...
        <div>
          ...
          {students.map((u) => (
            <div key={u.id} className="flex justify-between bg-white/10 p-3 mb-2 rounded">
              ...
              <button onClick={() => deleteUser(u.id)} className="bg-red-400 px-2 rounded">
                Delete
              </button>
            </div>
          ))}
        ...
        <div>
          ...
          {organisers.map((u) => (
            <div key={u.id} className="flex justify-between bg-white/10 p-3 mb-2 rounded">
              ...
              <div className="space-x-2">
                <button onClick={() => demoteUser(u.id)} className="bg-yellow-400 px-2 rounded">
                  Demote
                </button>
                <button onClick={() => deleteUser(u.id)} className="bg-red-400 px-2 rounded">
                  Delete
      ...
      <section className="w-[90%] mx-auto mt-10">
        ...
        <Link to="/create-event">
          <button className="bg-brand px-4 py-2 mb-4 rounded">
            + Create Event
          </button>
        </Link>
        ...
        {events.map((e) => (
          <div key={e.id} className="flex justify-between bg-white/10 p-3 mb-2 rounded">
            ...
            <div className="space-x-2">
              <button onClick={() => { setSelectedEvent(e); setShowEditModal(true); }} className="bg-yellow-400 px-2 rounded">
                Edit
              </button>

              <button onClick={() => deleteEvent(e.id)} className="bg-red-400 px-2 rounded">
                Delete
              </button>
            </div>
          </div>
